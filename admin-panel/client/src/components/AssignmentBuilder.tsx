...
export default function AssignmentBuilder({
  ...
  // Check if a saved local draft exists (only if not editing an existing assignment by id)
  const existingDraft = useMemo(() => {
    if (initialData?.id) return null;
    return getDraft<AssignmentData>("assignment");
  }, [initialData?.id]);
  ...
  const [isRestoredFromDraft, setIsRestoredFromDraft] = useState<boolean>(() => {
    ...
    return Boolean(
      existingDraft?.data &&
        (existingDraft.data.title || existingDraft.data.description || existingDraft.data.problemsList?.length)
    );
  });
  ...
  const initialProblems = initialData?.problemsList || existingDraft?.data?.problemsList || [];
  ...
  const [data, setData] = useState<AssignmentData>(() => {
    ...
    return {
      ...
      resources: [],
      submissionTypes: ["Code Editor / IDE", "ZIP / File upload", "GitHub repository link"],
      maxFileSize: "25 MB",
  ...
  // Auto-save form state to local draft when creating a new assignment
  useEffect(() => {
    if (data.id) return; // Do not overwrite drafts when editing an established assignment
    const hasData = Boolean(
      data.title.trim() ||
        data.description.trim() ||
        data.instructions.trim() ||
        data.problemsList.length > 0
    );
    if (!hasData) return;

    const timer = setTimeout(() => {
      saveDraft("assignment", data, {
